...
export function buildRawItems(f: TripFormState): RawItem[] {
  ...
  if (f.dest === 'playa') add('ropa', 'Ojotas');
  ...
  if (f.turismo === 'fiesta') add('ropa', 'Outfit para salir', 2);
  ...
  add('higiene', 'Afeitadora o pinza');
  ...
  if (f.motivo === 'trabajo') add('docs', 'Credencial de la empresa');
  ...
  if (f.transporte === 'avion' || f.transporte === 'bus') add('extras', 'Antifaz y tapones');
