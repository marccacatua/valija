...
export function Checklist() {
  ...
  const matchesFilter = (item: PackingItem) => {
    const matchesSearch = search.trim() === '' || itemLabel(item.name).toLowerCase().includes(search.trim().toLowerCase());
          ...
          >
            ...
            <span className={styles.qtyControls} onClick={(e) => e.stopPropagation()}>
              {!item.noQty && (
                <>
                  <button
                    ...
                    aria-label={t('Restar {item}', { item: itemLabel(item.name) })}
                  >
                    –
                  </button>
                  ...
                  <button
                    ...
                    onClick={() => bumpItem(trip.id, item.id, 1)}
                    aria-label={t('Sumar {item}', { item: itemLabel(item.name) })}
                  >
                    +
                  </button>
                </>
              ...
              <button
                ...
                aria-label={t('Borrar {item}', { item: itemLabel(item.name) })}
              >
                ×
              </button>
            </span>
    ...
    <div className={styles.homeSection}>
      ...
      {opts.sorted.map((task) => (
        <button key={task.id} ref={registerItemNode(task.id)} type="button" className={styles.item} onClick={() => opts.onToggle(task.id)}>
          ...
          <span className={`${styles.itemName} ${task.done ? styles.itemNameDone : ''}`}>{itemLabel(task.label)}</span>
          <span onClick={(e) => e.stopPropagation()}>
            <button type="button" className={styles.removeBtn} onClick={() => opts.onRemove(task)} aria-label={t('Borrar {item}', { item: itemLabel(task.label) })}>
              ×
            </button>
          </span>
        </button>
    ...
    return (
      ...
      >
        ...
        <span className={styles.quickGroupInfo}>
          ...
          <span className={styles.quickGroupHint}>{tn(list.length, '{n} ítem', '{n} ítems')}</span>
        </span>
        <span className={styles.groupCount}>
          {list.filter((i) => i.done).length}/{list.length}
        </span>
    ...
    <div className={styles.screen}>
      <div className={styles.hero}>
        ...
        <div className={styles.heroTop}>
          <button type="button" className={styles.backBtn} onClick={() => navigate('/viajes')} aria-label={t('Volver a mis viajes')}>
            {BackArrowIcon}
          </button>
          <div className={styles.heroLabel}>{t('Tu valija para')}</div>
        </div>
        {editingName ? (
          <input
            autoFocus
          ...
          <div className={styles.heroTitleRow}>
            ...
            <button type="button" className={styles.editNameBtn} onClick={startEditingName} aria-label={t('Cambiar nombre del viaje')}>
              {EditIcon}
            </button>
          </div>
        )}
        ...
        <div className={styles.progressCard}>
          <div style={{ flex: 1 }}>
            <div className={styles.progressCount}>
              ...
              <span className={styles.progressCountLabel}>{t('de {total} empacado', { total: items.length })}</span>
            </div>
      ...
      <div className={styles.groups}>
        {view === 'detallada' ? (
          groups.length > 0 ? (
            ...
            isFiltering && <div className={styles.noResults}>{t('No hay ítems que coincidan con la búsqueda.')}</div>
          )
        ) : (
        ...
        {boatChecklist.length > 0
          ? taskSection({
              title: t('¿Está todo listo para zarpar?'),
              hint: t('Seguridad y logística de la embarcación — no tiene nada que ver con la valija personal.'),
              tasks: boatChecklist,
          ...
          : taskSection({
              title: t('¿Quedó todo pronto en casa?'),
              hint: t('No suma al progreso de la valija — son cosas para dejar resueltas antes de salir.'),
              tasks: homeChecklist,
        ...
        {trip.form.maletas.length > 1 && (
          <Button onClick={() => navigate(`/viaje/${trip.id}/distribucion`)}>{t('Ver cómo repartir en tus valijas')}</Button>
        )}
        {canUseTemplates && (customItemsInTrip.length > 0 || customHomeTasksInTrip.length > 0) && (
          <Button variant="inverted" onClick={() => setSheet('save')}>
            {t('Guardar ítems como plantilla')}
          </Button>
        )}
        {canUseTemplates && (
        ...
        {!canAddCustomItems && (
          <Button variant="inverted" onClick={() => setShowPaywall(true)}>
            {LockIcon} {t('Desbloquear ítems propios, plantillas y repetir viaje')}
          </Button>
        )}
        {canExport && (
          <Button variant="inverted" onClick={handleShare}>
            {copied ? t('Copiado ✓') : t('Compartir checklist')}
          </Button>
        )}
      ...
      {templateToDelete && (
        <ConfirmDialog
          title={t('¿Borrar la plantilla "{name}"?', { name: templateToDelete.name })}
